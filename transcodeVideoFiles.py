...
def main():
    ...
    if options.transcode:
        ...
        for counter, f in enumerate(selected_files, 1):
            ...
            if not copy_video:
                cmd = 'ffmpeg -i \"%s\" -fps_mode passthrough %s -an -f %s -y /dev/null' % (f[0], video_options_1st_pass, fmt)
                print('')
                print(cmd)
                print('')
            ...
            cmd = 'ffmpeg -i \"%s\" -fps_mode passthrough %s %s -map_metadata 0 -metadata comment="%s" -y \"%s\"' % (f[0], video_options, audio_options, comment, dest_path)
            print('')
            print(cmd)
            print('')
            if not options.dry_run:
